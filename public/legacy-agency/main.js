...
function buildMailtoFallback(body, toEmail) {
  const enc = encodeURIComponent(body);
  const to = toEmail?.trim() || "aminennasri@outlook.com";
  return `mailto:${to}?subject=${encodeURIComponent("Brief — Friday")}&body=${enc}`;
}
...
function initFridayStartForm() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const targetEmail = form.getAttribute("data-formsubmit-email")?.trim() ?? "aminennasri@outlook.com";
    const textBody = [
      `Nom : ${name}`,
      `E-mail : ${email}`,
      `Téléphone : ${phone}`,
    ...
    if (mailtoLink && mailtoFallback) {
      mailtoLink.href = buildMailtoFallback(textBody, targetEmail);
