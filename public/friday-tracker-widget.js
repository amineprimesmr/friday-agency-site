...
//  Téléchargements & revenus réels · Données SensorTower
...
async function getSensorTower(ids) {
  const data = await fetchJSON(
...
async function buildSingleApp(widget, query) {
  ...
  const stMap = await getSensorTower([app.id]);
...
async function buildDashboard(widget, limit, family) {
  ...
  const stMap = apps.length > 0 ? await getSensorTower(apps.map(a => a.id)) : {};
