  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const data = (await res.json()) as TikTokOrganicContentResult & { error?: string };
        ...
        if (!res.ok) {
          setResult({
            ...
            error:
              data.error ??
              (res.status === 402 ? "Abonnement Trackapp requis." : "Impossible de charger TikTok pour l’instant."),
          });
          return;
        }
        setResult(data);
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex min-w-0 items-center gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 truncate text-[0.84rem] text-white/55">{profile?.handle ?? displayProfileUrl}</p>
            </div>
          </div>
          <a
